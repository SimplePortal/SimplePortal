...
{
	global $db_prefix, $context, $settings, $options, $txt;

	$query = array();
	if(!empty($column_id))
		$query[] = "spb.col = '$column_id'";
	if(!empty($block_id))
		$query[] = "spb.ID_BLOCK = '$block_id'";
	if(!empty($state))
		$query[] = "spb.state = 1";
	...
	$request = db_query("
		...
		ORDER BY spb.col, spb.row", __FILE__, __LINE__);
	$return = array();
...
function getShowInfo($block_id = null, $display = null, $custom = null)
{
	...
	if ($display === null || $custom === null)
	{
		...
		list ($display, $custom) = mysql_fetch_row($request);
		mysql_free_result($request);
	}
...
function sp_allowed_to($type, $id, $set = null, $allowed = null, $denied = null)
{
	...
	if (!isset($set, $allowed, $denied))
	{
		$request = db_query("
			...
			FROM {$db_prefix}sp_{$types[$type]['table']}
			WHERE {$types[$type]['id']} = $id
			LIMIT 1", __FILE__, __LINE__);
		list ($set, $allowed, $denied) = mysql_fetch_row($request);
		mysql_free_result($request);
	}
...
function sp_query_string($tourniquet)
{
	...
	$fix = str_replace('{version}', $sportal_version, '<a href="https://simpleportal.net/" target="_blank">SimplePortal {version} &copy; 2008-' . strftime('%Y') . ', SimplePortal</a>');
	...
	if (isset($_GET['area']) && $_GET['area'] == 'portalpages')
...
function sp_loadCalendarData($type, $low_date, $high_date = false)
{
	...
	if (!isset($loaded))
	{
		...
		$loaded = array(
			'getEvents' => 'calendarEventArray',
			'getBirthdays' => 'calendarBirthdayArray',
			'getHolidays' => 'calendarHolidayArray',
		);
	}
...
{
	global $color_profile, $scripturl, $modSettings, $db_prefix;
	...
	while ($row = mysql_fetch_assoc($request))
	{
		...
		$color_profile[$row['ID_MEMBER']]['link'] = '<a href="' . $scripturl . '?action=profile;u=' . $row['ID_MEMBER'] . '"' . (!empty($onlineColor) ? ' style="color: ' . $onlineColor . ';"' : '') . '>' . $row['realName'] . '</a>';
		$color_profile[$row['ID_MEMBER']]['colored_name'] = (!empty($onlineColor) ? '<span style="color: ' . $onlineColor . ';">' : '' ) . $row['realName'] . (!empty($onlineColor) ? '</span>' : '');
	}
	mysql_free_result($request);
	...
	if ($action == 'implode')
	{
		...
		foreach ($style_parameters as $parameter)
			if (isset($_POST[$parameter]))
				$style .= $parameter . '~' . $func['htmlspecialchars']($func['htmltrim']($_POST[$parameter]), ENT_QUOTES) . '|';
			else
				$style .= $parameter . '~|';
...
{
	global $db_prefix, $func, $scripturl, $context, $user_info, $user_settings, $ID_MEMBER, $txt;
		...
		while ($row = mysql_fetch_assoc($request))
		{
			...
			$shouts[$row['ID_SHOUT']] = array(
				...
				'author' => array(
					'id' => $row['ID_MEMBER'],
					'name' => $row['memberName'],
					'link' => $row['ID_MEMBER'] ? ('<a href="' . $scripturl . '?action=profile;u=' . $row['ID_MEMBER'] . '" title="' . $txt[30] . ' ' . strip_tags(timeformat($row['log_time'])) . '"' . (!empty($online_color) ? ' style="color: ' . $online_color . ';"' : '') . '>' . $row['memberName'] . '</a>') : $row['memberName'],
					'color' => $online_color,
				),
				'time' => $row['log_time'],
	...
	{
		if (preg_match('~^@(.+?): ~' . ($context['utf8'] ? 'u' : ''), $shout['text'], $target) && $func['strtolower']($target[1]) !== $func['strtolower']($user_info['name']) && $shout['author']['id'] != $ID_MEMBER && !$user_info['is_admin'])
		{
		...
		$shouts[$shout['id']] += array(
			...
			'delete_link' => $can_delete ? '<a href="' . $scripturl . '?action=portal;sa=shoutbox;shoutbox_id=' . $shoutbox . ';delete=' . $shout['id'] . ';sesc=' . $context['session_id'] . '">' . sp_embed_image('delete_small') . '</a> ' : '',
			'delete_link_js' => $can_delete ? '<a href="' . $scripturl . '?action=portal;sa=shoutbox;shoutbox_id=' . $shoutbox . ';delete=' . $shout['id'] . ';sesc=' . $context['session_id'] . '" onclick="sp_delete_shout(' . $shoutbox . ', ' . $shout['id'] . ', \'sc\', \'' . $context['session_id'] . '\'); return false;">' . sp_embed_image('delete_small') . '</a> ' : '',
		);
		...
		if (!empty($user_settings['pm_ignore_list']) && in_array($shout['author']['id'], explode(',', $user_settings['pm_ignore_list'])))
...
function sportal_create_shout($shoutbox, $shout)
{
	...
	db_query("
		INSERT IGNORE INTO {$db_prefix}sp_shouts
			(ID_SHOUTBOX, ID_MEMBER, memberName, log_time, body)
		VALUES ($shoutbox[id], $ID_MEMBER, '$user_info[name]', " . time() . ", '$shout')", __FILE__, __LINE__);
	...
	if ($shoutbox['num_shouts'] > $shoutbox['num_max'])
	{
		$request = db_query("
			...
			WHERE ID_SHOUTBOX = $shoutbox[id]
			ORDER BY log_time
			LIMIT " . ($shoutbox['num_shouts'] - $shoutbox['num_max']), __FILE__, __LINE__);
		$old_shouts = array();
		while ($row = mysql_fetch_assoc($request))
...
function sportal_delete_shout($shoutbox_id, $shouts, $prune = false)
{
	...
	db_query("
		...
		WHERE ID_SHOUT IN (" . implode(', ', $shouts) . ")", __FILE__, __LINE__);
...
function sportal_update_shoutbox($shoutbox_id, $num_shouts = 0)
{
	...
	db_query("
		...
		SET last_update = " . time() . ($num_shouts === 0 ? "" : ",
			num_shouts = " . ($num_shouts === true ? "num_shouts + 1" : "num_shouts - " . $num_shouts)) . "
...
{
	global $db_prefix, $modSettings, $user_info, $txt;
	...
	{
		if ($fatal)
			fatal_lang_error('error_sp_flood_' . $type, false, array($modSettings['spamWaitTime']));
		else
			return isset($txt['error_sp_flood_' . $type]) ? sprintf($txt['error_sp_flood_' . $type], $modSettings['spamWaitTime']) : true;
	}
